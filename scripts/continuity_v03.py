#!/usr/bin/env python3
"""Agent Continuity v0.3: intent capture, invariants, impact staleness, and fresh review."""
...
TERMINAL_SOURCE_DISPOSITIONS = {"deferred", "waived", "superseded", "rejected"}
...
def _initial_finding_status(source: dict[str, Any], disposition: str) -> str:
    ...
    with base.connect(db) as conn:
        ...
        for source in manifest.get("sources") or []:
            ...
            if kind in {"reviewer", "defect"} or key.startswith("FND-"):
                old = conn.execute("SELECT status FROM review_findings WHERE finding_key=?", (key,)).fetchone()
                if disposition in TERMINAL_SOURCE_DISPOSITIONS:
                    status = disposition
                elif old:
                ...
                conn.execute(
                    """INSERT INTO review_findings(finding_key,phase_key,severity,title,description,status,discovered_commit,created_at,updated_at)
                       VALUES(?,?,?,?,?,?,?,?,?)
                       ON CONFLICT(finding_key) DO UPDATE SET phase_key=excluded.phase_key,severity=excluded.severity,
                         title=excluded.title,description=excluded.description,status=excluded.status,
                         discovered_commit=COALESCE(review_findings.discovered_commit,excluded.discovered_commit),updated_at=excluded.updated_at""",
        ...
        for inv in manifest.get("invariants") or []:
            ...
            conn.execute(
                ...
                (key, phase, str(inv.get("description") or inv.get("title") or key),
                 str(inv.get("severity") or "P1"), status, stamp, stamp),
            )
    ...
    with base.connect(db) as conn:
        ...
        for source in conn.execute("SELECT * FROM scope_sources WHERE phase_key=? AND active=1 ORDER BY source_key", (phase,)):
            ...
            if disposition == "unmapped":
                ...
                active_mapping = conn.execute(
                    """SELECT 1 FROM source_requirements sr
                       JOIN requirements r ON r.requirement_key=sr.requirement_key
                       WHERE sr.source_key=? AND r.active=1 LIMIT 1""",
                    (source["source_key"],),
                ).fetchone()
                if not active_mapping:
                    failures.append({"id": source["source_key"], "reason": "mapped source has no active current requirement mapping"})
            elif disposition in TERMINAL_SOURCE_DISPOSITIONS and not source["reason"]:
        ...
        for req in conn.execute("SELECT * FROM requirements WHERE phase_key=? AND active=1 AND disposition='required'", (phase,)):
            if not conn.execute(
                "SELECT 1 FROM acceptance_checks WHERE requirement_key=? AND active=1 AND required=1",
                (req["requirement_key"],),
            ).fetchone():
        ...
        for inv in conn.execute("SELECT * FROM invariants WHERE phase_key=? AND active=1 AND status='active'", (phase,)):
            mapped = conn.execute(
                """SELECT 1 FROM check_impacts ci
                   JOIN acceptance_checks ac ON ac.check_key=ci.check_key
                   JOIN requirements r ON r.requirement_key=ac.requirement_key
                   WHERE ci.kind='invariant' AND ci.value=? AND ac.active=1 AND ac.required=1
                     AND r.active=1 AND r.disposition='required' LIMIT 1""",
                (inv["invariant_key"],),
            ).fetchone()
            if not mapped:
                failures.append({"id": inv["invariant_key"], "reason": "active invariant has no required current check mapping"})
        ...
        base._event(conn, "CAPTURE_GATE_PASSED" if status == "passed" else "CAPTURE_GATE_FAILED",
                    f"Scope capture gate {status} for {phase}", {"gate_id": gate_id, "failures": failures})
...
def evidence(db: Path, *, check_keys: list[str], kind: str, status: str, value: str,
             commit: str | None, task: str | None, workspace: str | None, artifact: str | None) -> int:
    eid = base.record_check_evidence(
        db, check_keys=check_keys, kind=kind, status=status, value=value,
        commit_sha=commit, task_key=task, workspace_id=workspace, artifact_ref=artifact,
    )
    if status == "passed":
        with base.connect(db) as conn:
    ...
    with base.connect(db) as conn:
        for row in conn.execute(
            """SELECT DISTINCT ci.check_key,ci.value FROM check_impacts ci
               JOIN acceptance_checks ac ON ac.check_key=ci.check_key
               JOIN requirements r ON r.requirement_key=ac.requirement_key
               WHERE ci.kind='module' AND ac.active=1 AND r.active=1 AND r.phase_key=?""",
            (phase,),
        ):
        ...
        for key in stale:
            conn.execute(
                """INSERT INTO check_staleness(check_key,stale,reason,changed_ref,updated_at) VALUES(?,1,?,?,?)
                   ON CONFLICT(check_key) DO UPDATE SET stale=1,reason=excluded.reason,
                     changed_ref=excluded.changed_ref,updated_at=excluded.updated_at""",
                (key, f"impacted by module change: {', '.join(modules)}", changed_ref, base.now()),
            )
        if stale:
            base._event(conn, "EVIDENCE_STALE", f"Marked {len(stale)} checks stale", {
                "checks": sorted(stale), "modules": modules, "changed_ref": changed_ref,
            })
    return sorted(stale)
...
def gate(db: Path, phase: str, commit: str, require_pr: bool = True) -> dict[str, Any]:
    ...
    with base.connect(db) as conn:
        ...
        if not manifest:
            ...
            for row in conn.execute(
                """SELECT cs.check_key FROM check_staleness cs
                   JOIN acceptance_checks ac ON ac.check_key=cs.check_key
                   JOIN requirements r ON r.requirement_key=ac.requirement_key
                   WHERE cs.stale=1 AND ac.active=1 AND ac.required=1 AND r.active=1
                     AND r.disposition='required' AND r.phase_key=?""",
                (phase,),
            ):
    ...
    if failures:
        with base.connect(db) as conn:
            cur = conn.execute(
                "INSERT INTO gate_runs(phase_key,manifest_hash,commit_sha,status,failures_json,created_at) VALUES(?,?,?,'failed',?,?)",
                (phase, digest, commit, json.dumps(failures, sort_keys=True), base.now()),
            )
            base._event(conn, "GATE_FAILED", f"v0.3 completion gate failed for {phase}", {
                "gate_id": int(cur.lastrowid), "failures": failures,
            })
    ...
    if result["status"] == "passed":
        with base.connect(db) as conn:
            conn.execute(
                "UPDATE invariants SET last_verified_commit=?,updated_at=? WHERE phase_key=? AND active=1 AND status='active'",
                (commit, base.now(), phase),
            )
    return result


def register_finding(db: Path, key: str, phase: str, severity: str,
                     title: str, description: str, commit: str | None) -> None:
    base.init(db)
    stamp = base.now()
    with base.connect(db) as conn:
        conn.execute(
            """INSERT INTO scope_sources(source_key,phase_key,kind,summary,disposition,active,created_at,updated_at)
               VALUES(?,?,?,?, 'unmapped',1,?,?)
               ON CONFLICT(source_key) DO UPDATE SET phase_key=excluded.phase_key,kind='reviewer',
                 summary=excluded.summary,disposition='unmapped',active=1,updated_at=excluded.updated_at""",
            (key, phase, "reviewer", title, stamp, stamp),
        )
        conn.execute(
            """INSERT INTO review_findings(finding_key,phase_key,severity,title,description,status,discovered_commit,created_at,updated_at)
               VALUES(?,?,?,?,?,'open',?,?,?)
               ON CONFLICT(finding_key) DO UPDATE SET severity=excluded.severity,title=excluded.title,
...
def _completion_gate_exists(conn, phase: str, commit: str) -> bool:
    manifest = base._manifest_row(conn, phase)
    if not manifest:
        return False
...
def review_gate(db: Path, phase: str, commit: str, new_findings: Iterable[str]) -> dict[str, Any]:
    ...
    with base.connect(db) as conn:
        unresolved = [
            row["finding_key"]
            for row in conn.execute("SELECT finding_key,status FROM review_findings WHERE phase_key=?", (phase,))
        ...
        gate_id = int(cur.lastrowid)
        base._event(conn, "FRESH_REVIEW_PASSED" if status == "passed" else "FRESH_REVIEW_FAILED",
                    f"Fresh reviewer gate {status} for {phase}", {"gate_id": gate_id, "unresolved": unresolved})
    return {"gate_id": gate_id, "phase": phase, "commit": commit, "status": status,
            "new_findings": keys, "unresolved_findings": unresolved}
...
def task(db: Path, key: str, phase_key: str, title: str, status: str,
         acceptance: str | None, branch: str | None, pr_number: int | None, commit: str | None) -> None:
    if status == "completed":
...
def bootstrap(db: Path) -> dict[str, Any]:
    ...
    phase_key = project.get("current_phase")
    if not phase_key:
        return result
    with base.connect(db) as conn:
        result["scope_v03"] = {
            ...
            "stale_checks": [dict(r) for r in conn.execute(
                """SELECT cs.* FROM check_staleness cs JOIN acceptance_checks ac ON ac.check_key=cs.check_key
                   JOIN requirements r ON r.requirement_key=ac.requirement_key WHERE cs.stale=1 AND r.phase_key=?""",
                (phase_key,),
            )],
            "last_capture_gate": (lambda r: dict(r) if r else None)(conn.execute(
                "SELECT * FROM scope_capture_gate_runs WHERE phase_key=? ORDER BY id DESC LIMIT 1", (phase_key,),
            ).fetchone()),
            "last_review_gate": (lambda r: dict(r) if r else None)(conn.execute(
                "SELECT * FROM fresh_review_gate_runs WHERE phase_key=? ORDER BY id DESC LIMIT 1", (phase_key,),
            ).fetchone()),
        }
    return result
...
def main() -> None:
    ...
    try:
        if args.command == "init":
            ...
            phase(args.db, args.key, args.title, args.status, args.commit)
        elif args.command == "task":
            task(args.db, args.key, args.phase, args.title, args.status, args.acceptance, args.branch, args.pr_number, args.commit)
        elif args.command == "pr":
            base.set_task_pr(args.db, args.task, args.number)
        elif args.command == "evidence":
            if not args.check: raise base.ContinuityError("v0.3 evidence requires at least one --check")
            print(json.dumps({"evidence_id": evidence(args.db, check_keys=args.check, kind=args.kind,
                status=args.status, value=args.value, commit=args.commit, task=args.task,
                workspace=args.workspace, artifact=args.artifact)}))
        elif args.command == "impact":
            print(json.dumps({"stale_checks": impact(args.db, args.phase, args.module, args.changed_ref)}, indent=2))
        elif args.command == "finding":
            register_finding(args.db, args.key, args.phase, args.severity, args.title, args.description, args.commit)
        elif args.command == "finding-state":
            finding_state(args.db, args.key, args.status, args.reason, args.destination)
            ...
            base.checkpoint(args.db, args.phase, args.summary, args.commit)
    except base.ContinuityError as exc:
        p.error(str(exc))
