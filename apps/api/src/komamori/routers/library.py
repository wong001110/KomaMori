...
def create_chapter(series_id: int, payload: ChapterCreate, session: Session = Depends(get_session)) -> Chapter:
    ...
    existing = session.scalar(
        select(Chapter).where(Chapter.series_id == series_id, Chapter.number == payload.number)
    )
    if existing:
...
async def import_chapter_pages(
    chapter_id: int,
    ...
    for page_index, import_page in enumerate(imported, start=1):
        ...
        session.add(
            Page(
                ...
                original_asset=relative,
                width=import_page.width,
                height=import_page.height,
...
def list_regions(page_id: int, session: Session = Depends(get_session)) -> list[TextRegion]:
    ...
    return list(
        session.scalars(
            select(TextRegion).where(TextRegion.page_id == page_id).order_by(TextRegion.reading_order, TextRegion.id)
        )
    )
