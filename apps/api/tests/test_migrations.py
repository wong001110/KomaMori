...
def test_fresh_database_is_initialized_and_versioned(tmp_path: Path) -> None:
    ...
    with engine.connect() as connection:
        row = connection.exec_driver_sql("SELECT version, name FROM schema_migrations").one()
        assert tuple(row) == (1, "structured-mvp-baseline")
...
def test_existing_database_is_adopted_without_losing_rows(tmp_path: Path) -> None:
    ...
    with engine.connect() as connection:
        assert connection.exec_driver_sql("SELECT COUNT(*) FROM schema_migrations WHERE version=1").scalar_one() == 1
