...
export type Chapter = { id: number; series_id: number; title: string; number: number; display_number: string; sort_order: number; status: string };
...
export type Localization = { id: number; text_region_id: number; locale: string; text: string; status: "draft" | "needs-review" | "approved" | string; source: string; provenance: Record<string, unknown>; quality_metadata: Record<string, unknown>; layout: Record<string, unknown> };
export type RegionView = { id: number; page_id: number; region_type: RegionType | string; geometry: number[][]; source_text: string; ocr_confidence: number | null; ocr_provenance: Record<string, unknown>; reading_order: number; localization: Localization | null };
...
export type ChapterView = { chapter_id: number; series_id: number; title: string; number: number; display_number: string; sort_order: number; locale: string; pages: PageView[] };
...
export const api = {
  ...
  createChapter: (seriesId: number, title: string, displayNumber: string, sortOrder: number) => request<Chapter>(`/api/series/${seriesId}/chapters`, { method: "POST", ...json({ title, display_number: displayNumber, sort_order: sortOrder }) }),
  getChapter: (id: number) => request<ChapterDetail>(`/api/chapters/${id}`),
  updateChapter: (id: number, patch: Partial<Pick<Chapter, "title" | "display_number" | "sort_order">>) => request<Chapter>(`/api/chapters/${id}`, { method: "PATCH", ...json(patch) }),
